module Platon
  class Client
    ...
    #PLATON_DELETE_COMMANDS =  %w(platon_coinbase ,plton_mining,platon_hashrate,platon_getUncleCountByBlockHash,platon_getUncleCountByBlockNumber,platon_getUncleByBlockNumberAndIndex platon_getCompilers platon_compileLLL platon_compileSolidity platon_compileSerpent)
    ...
    attr_accessor :command, :id, :log, :logger, :default_account, :gas_price, :gas_limit, :ppos
    ...
    def default_account
      @default_account ||= platon_accounts["result"][0]
    end
    ...
    def get_balance(address)
      platon_get_balance(address)["result"].to_i(16)
    end
    ...
    def get_nonce(address)
      platon_get_transaction_count(address, "pending")["result"].to_i(16)
    end
    ...
    def transfer(key, address, amount)
      Platon.configure { |c| c.chain_id = net_version["result"].to_i }
      args = { 
        from: key.address,
        to: address,
        value: amount,
      ...
      tx.sign key
      platon_send_raw_transaction(tx.hex)["result"]
    end
    ...
    def send_command(command,args)
      # if ["platon_getBalance", "platon_call"].include?(command)
      #   args << "latest"
      # end
